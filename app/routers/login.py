...
async def login(user: UserLogin, response: Response) -> dict:
    ...
    if (user.login == 'user' and user.password == 'user') or (user.login == 'admin' and user.password == 'admin'):
        ...
        msg = {'result': '400', 'message': 'Incorrect password for user, pasword must be admin'}
        logger.error(f'Requested user {user.login} login failed. {response.status_code} {msg}')
        return msg
    elif user.login == 'admin':
        response.status_code = 403
        msg = {'result': '400', 'message': 'Incorrect pasword for admin, password must be user'}
        logger.error(f'Requested user {user.login} login failed. {response.status_code} {msg}')
        return msg
